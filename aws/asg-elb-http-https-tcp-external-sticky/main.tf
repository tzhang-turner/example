...
resource "aws_elb" "elb-tf" {
  ...
  security_groups = ["${split(",", var.lb_specific_security_groups == "" ? lookup(var.aws_external_security_groups, var.aws_account) : var.lb_specific_security_groups)}"]
  ...
  listener {
    instance_port     = "${var.elb_tcp_listener_instance_port}"
    instance_protocol = "tcp"
    lb_port           = "${var.elb_tcp_listener_lb_port}"
    lb_protocol       = "tcp"
  }
...
resource "aws_lb_cookie_stickiness_policy" "elb-tf-cookie" {
  ...
  load_balancer            = "${aws_elb.elb-tf.id}"
  lb_port                  = "${var.elb_sticky_lb_port}"
  cookie_expiration_period = "${var.elb_cookie_expiration}"
}

resource "aws_lb_cookie_stickiness_policy" "elb-tf-cookie-https" {
  name                     = "sticky-session-policy"
  load_balancer            = "${aws_elb.elb-tf.id}"
  lb_port                  = 443
  cookie_expiration_period = "${var.elb_cookie_expiration}"
