...
resource "aws_launch_configuration" "lc-tf" {
  name_prefix                 = "ext-${var.name}-${var.environment}-tf-"
  image_id                    = "${var.aws_ami}"
  instance_type               = "${var.aws_instance_type}"
  security_groups             = ["${split(",", var.custom_external_security_groups == "" ? lookup(var.aws_external_security_groups, var.aws_account) : var.custom_external_security_groups)}"]
...
resource "aws_autoscaling_group" "asg-tf" {
  ...
  vpc_zone_identifier  = ["${lookup(var.aws_external_zone_identifiers, var.aws_account)}"]
