...
resource "aws_elb" "elb-tf" {
  ...
  security_groups = ["${split(",", var.elb_specific_security_groups == "" ? lookup(var.aws_security_groups, var.aws_account) : var.elb_specific_security_groups)}"]
  subnets         = ["${split(",", var.elb_internal_bool ? lookup(var.aws_zone_identifiers, var.aws_account) : lookup(var.aws_external_zone_identifiers, var.aws_account) )}"]
